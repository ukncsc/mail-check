...
{
    public interface ITls11AvailableWithBestCipherSuiteSelected : ITlsEvaluator { }

    public class Tls11AvailableWithBestCipherSuiteSelected : ITls11AvailableWithBestCipherSuiteSelected
    {
        ...
        private readonly string intro = "When testing TLS 1.1 with a range of cipher suites";
        ...
        public TlsEvaluatorResult Test(TlsConnectionResult tlsConnectionResult)
        {
            ...
            switch (tlsConnectionResult.CipherSuite)
            {
                ...
                case CipherSuite.TLS_RSA_WITH_AES_128_CBC_SHA:
                    return new TlsEvaluatorResult(EvaluatorResult.WARNING, $"{intro} the server selected a cipher suite with no Perfect Forward Secrecy (PFS). {advice}");

                case CipherSuite.TLS_RSA_WITH_3DES_EDE_CBC_SHA:
                case CipherSuite.TLS_DH_DSS_WITH_3DES_EDE_CBC_SHA:
                case CipherSuite.TLS_DH_RSA_WITH_3DES_EDE_CBC_SHA:
                    return new TlsEvaluatorResult(EvaluatorResult.WARNING, $"{intro} the server selected a cipher suite with no Perfect Forward Secrecy (PFS) and that uses 3DES. {advice}");

                case CipherSuite.TLS_RSA_WITH_RC4_128_SHA:
                    return new TlsEvaluatorResult(EvaluatorResult.WARNING, $"{intro} the server selected a cipher suite with no Perfect Forward Secrecy (PFS) and that uses RC4. {advice}");
                ...
                case CipherSuite.TLS_DHE_RSA_EXPORT_WITH_DES40_CBC_SHA:
                    return new TlsEvaluatorResult(EvaluatorResult.FAIL, $"{intro} the server selected a insecure cipher suite. {advice}");
            }
