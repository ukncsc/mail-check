...
{
    [TestFixture]
    public class RufTagsShouldContainDmarcServiceMailBoxTest
    {
        private RufTagsShouldContainDmarcServiceMailBox _rule;

        [SetUp]
        public void SetUp()
        {
            _rule = new RufTagsShouldContainDmarcServiceMailBox();
        }
        ...
        public void ErrorWhenIncorrectMailCheckMailbox()
        {
            var testDmarcRecord = CreateDmarcRecord(
                CreateReportUriAggregate(
                    new Uri("mailto:ruf@dmarc.service.gov.uk"),
                    new Uri("mailto:ruf@test.service.gov.uk")));
        ...
        public void NoErrorWhenDuplicateTags()
        {
            var testDmarcRecord = CreateDmarcRecord(
                CreateReportUriAggregate(
                    new Uri("mailto:ruf@test.service.gov.uk")),
                CreateReportUriAggregate(
                    new Uri("mailto:dmarc-ruf@dmarc.service.gov.uk")));
        ...
        [Test]
        public void ErrorWhenMailCheckRuaMailboxUsed()
        {
            var testdmDmarcRecord = CreateDmarcRecord(
                CreateReportUriAggregate(new Uri("mailto:dmarc-rua@dmarc.service.gov.uk")));

            Test(testdmDmarcRecord, true, ErrorType.Error);
        }
        ...
        [Test]
        public void NoErrorWhenCorrectMailboxUsed()
        {
            var testDmarcRecord = CreateDmarcRecord(
        ...
        public void NoExceptionWhenNullUri()
        {
            var testDmarc = CreateDmarcRecord(
                CreateReportUriAggregate(new[] { new Uri("mailto:dmarc-ruf@dmarc.service.gov.uk"), null }));
        ...
        private static ReportUriForensic CreateReportUriAggregate(params Uri[] uris)
        {
            return new ReportUriForensic("", uris?.Select(_ => new UriTag("", new DmarcUri(_), new MaxReportSize(1000, Unit.K))).ToList());
        }
    }
}
