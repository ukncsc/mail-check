...
namespace Dmarc.ForensicReport.Parser.Lambda.Test.Dao
{
    ...
    public class OriginalRcptToDaoTests : DatabaseTestBase
    {
        private OriginalRcptToDao _originalRcptToDao;
        ...
        protected override void SetUp()
        {
            ...
            _originalRcptToDao = new OriginalRcptToDao(_emailAddressDao);
        }
        ...
        public async Task AddOriginalMailFromWhenDoesntExistCorrectlyAdded()
        {
            ...
            using (MySqlConnection connection = new MySqlConnection(ConnectionString))
            {
                ...
                {
                    emailAddressReportsFromDao = await _originalRcptToDao.Add(new List<EmailAddressReportEntity> { emailAddressReport }, connection, transaction);
                    transaction.Commit();
                }
                connection.Close();
            }
            ...
            int count = 0;
            using (DbDataReader reader = MySqlHelper.ExecuteReader(ConnectionString, "SELECT * FROM forensic_report_rcpt_to"))
            {
                while (reader.Read())
                {
                    count++;
                    ...
                    Assert.That(reader.GetInt64("rcpt_to_id"), Is.EqualTo(emailAddressReportsFromDao[0].EmailAddressEntity.Id));
        ...
        public async Task AddTwiceThrowsException()
        {
            ...
            using (MySqlConnection connection = new MySqlConnection(ConnectionString))
            {
                ...
                using (MySqlTransaction transaction = await connection.BeginTransactionAsync().ConfigureAwait(false))
                {
                    await _originalRcptToDao.Add(new List<EmailAddressReportEntity> { emailAddressReport }, connection, transaction);
                    Assert.ThrowsAsync<MySqlException>(async () => await _originalRcptToDao.Add(new List<EmailAddressReportEntity> { emailAddressReport }, connection, transaction));
                    transaction.Commit();
                }
                connection.Close();
            }
