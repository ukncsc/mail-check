...
CREATE PROCEDURE import_new_providers()
BEGIN
...
INSERT INTO `derived_job_semaphore` (job_type, start_time) VALUES ('import_providers',@started_at);
...
UPDATE `derived_job_semaphore` SET end_time=CURRENT_TIMESTAMP where `job_type` = 'import_providers' and `start_time` = @started_at;
END IF;
...
DROP EVENT IF EXISTS import_new_providers_daily;
