...
DROP TABLE IF EXISTS `derived_aggregate_top10_daily_threshold`;

CREATE TABLE IF NOT EXISTS `derived_aggregate_top10_daily_threshold` (
...
DROP TABLE IF EXISTS `derived_aggregate_daily_senders_top10`;

CREATE TABLE IF NOT EXISTS `derived_aggregate_daily_senders_top10` (
  `domain_id` INT UNSIGNED NOT NULL,
  `effective_date` DATE NOT NULL,
...
DROP PROCEDURE IF EXISTS generate_derived_aggregate_daily_senders_top10;
DROP PROCEDURE IF EXISTS update_derived_aggregate_daily_senders_top10;

delimiter |

CREATE PROCEDURE update_derived_aggregate_daily_senders_top10()
BEGIN
...
INSERT INTO `derived_job_semaphore` (job_type, start_time) VALUES ('top10_senders',@started_at);


REPLACE INTO derived_aggregate_top10_daily_threshold 
SELECT
...
CAST(COALESCE((SELECT GREATEST(SUBSTRING_INDEX(SUBSTRING_INDEX(GROUP_CONCAT(d2.full_compliance_count ORDER BY d2.full_compliance_count DESC SEPARATOR ','), ',',10),',',-1),1)
   FROM derived_aggregate_all_daily_senders d2 
...
FROM `derived_aggregate_all_daily_senders`  d1
WHERE effective_date >= COALESCE((SELECT DATE_SUB(max(effective_date), INTERVAL 2 DAY) FROM derived_aggregate_top10_daily_threshold),
(SELECT MIN(effective_date) FROM aggregate_report))
GROUP BY effective_date, domain_id;


REPLACE INTO `derived_aggregate_daily_senders_top10` 
SELECT
d1.domain_id domain_id,
d1.effective_date effective_date,
...
FROM `derived_aggregate_all_daily_senders`  d1
JOIN `derived_aggregate_top10_daily_threshold` using(effective_date,domain_id) 
where effective_date >= COALESCE((SELECT DATE_SUB(max(effective_date), INTERVAL 2 DAY) FROM derived_aggregate_daily_senders_top10),
(SELECT MIN(effective_date) FROM aggregate_report)) 
...
UPDATE `derived_job_semaphore` SET end_time=CURRENT_TIMESTAMP where `job_type` = 'top10_senders' and `start_time` = @started_at;
