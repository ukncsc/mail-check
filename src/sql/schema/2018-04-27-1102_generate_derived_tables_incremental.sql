CREATE TABLE IF NOT EXISTS `derived_aggregate_all_daily_senders_20180427` (
  ...
  INDEX (source_ip),
  CONSTRAINT `derived_aggregate_all_daily_senders_20180427$provider_id`
...
CREATE TABLE IF NOT EXISTS `derived_aggregate_daily_20180427` (
  `domain_id` INT UNSIGNED NOT NULL,
  `effective_date` DATE NOT NULL,
  `full_compliance_count`INT UNSIGNED NOT NULL,
  `dkim_only_count` INT UNSIGNED NOT NULL,
...
CREATE PROCEDURE update_derived_aggregate_all_daily_senders_20180427()
BEGIN
...
INSERT INTO `derived_job_semaphore_20180427` (job_type, start_time) VALUES ('derived_aggregate_all_daily_senders',@started_at);

REPLACE INTO `derived_aggregate_all_daily_senders_20180427` SELECT r.header_from_domain_id domain_id,effective_date,r.source_ip,
provider_id,
...
COALESCE((SELECT DATE_SUB(max(effective_date), INTERVAL 2 DAY) FROM derived_aggregate_all_daily_senders_20180427),
(SELECT MIN(effective_date) FROM aggregate_report))
AND
COALESCE((SELECT effective_date FROM aggregate_report  WHERE effective_date >= COALESCE((SELECT DATE_SUB(max(effective_date), INTERVAL 2 DAY) FROM derived_aggregate_all_daily_senders_20180427),
...
UPDATE `derived_job_semaphore_20180427` SET end_time=CURRENT_TIMESTAMP WHERE `job_type` = 'derived_aggregate_all_daily_senders' AND `start_time` = @started_at;
...
CREATE PROCEDURE update_derived_aggregate_daily_20180427()
BEGIN
DECLARE job_running int;
DECLARE started_at timestamp;
...
INSERT INTO `derived_job_semaphore_20180427` (job_type, start_time) VALUES ('derived_aggregate_daily',@started_at);


REPLACE INTO `derived_aggregate_daily_20180427` 
...
FROM `derived_aggregate_all_daily_senders_20180427` daads
WHERE effective_date >=
COALESCE((SELECT DATE_SUB(max(effective_date), INTERVAL 2 DAY) FROM derived_aggregate_daily_20180427),
(SELECT MIN(effective_date) FROM aggregate_report))
group by domain_id,effective_date;


UPDATE `derived_job_semaphore_20180427` SET end_time=CURRENT_TIMESTAMP where `job_type` = 'derived_aggregate_daily' and `start_time` = @started_at;
END IF;
