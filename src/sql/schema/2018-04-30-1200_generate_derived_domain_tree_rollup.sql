...
CREATE PROCEDURE generate_derived_domain_tree()
BEGIN
...
INSERT INTO `derived_job_semaphore` (job_type, start_time) VALUES ('derived_domain_tree',@started_at);

DROP TABLE IF EXISTS domain_elements;

CREATE TABLE domain_elements AS 
...
ALTER TABLE domain_elements add PRIMARY KEY (id,element);
ALTER TABLE domain_elements add index idx_element (element);

DROP TABLE IF EXISTS derived_domain_tree;

CREATE TABLE derived_domain_tree AS SELECT cd.id child_id, cd.name child_name,IF(cd.monitor=b'1' OR cd.publish=b'1',b'1',b'0') child_visible, pd.id parent_id, pd.name parent_name, IF(pd.monitor=b'1' OR pd.publish=b'1',b'1',b'0') parent_visible,
pd.name regexp '^[a-z0-9]+\.(((ac|co|gov|ltd|me|net|nhs|org|plc|police|sch)\.uk)|com|org)$' parent_orgdomain
FROM domain pd
JOIN domain_elements child_elements ON child_elements.element = pd.name
JOIN domain cd ON child_elements.id=cd.id;

ALTER TABLE derived_domain_tree add PRIMARY KEY (child_id,parent_id);



UPDATE `derived_job_semaphore` SET end_time=CURRENT_TIMESTAMP where `job_type` = 'derived_domain_tree' and `start_time` = @started_at;
