...
CREATE TABLE IF NOT EXISTS `derived_aggregate_daily_senders_top10_rollup` (
...
CREATE PROCEDURE update_derived_aggregate_daily_senders_top10_rollup()
BEGIN
...
INSERT INTO `derived_job_semaphore` (job_type, start_time) VALUES ('derived_aggregate_daily_senders_top10_rollup',@started_at);


REPLACE INTO `derived_aggregate_daily_senders_top10_rollup` 
...
FROM `derived_aggregate_daily_senders_top10` dad
JOIN derived_domain_tree ddt ON ddt.child_id = dad.domain_id
WHERE effective_date BETWEEN COALESCE((SELECT DATE_SUB(max(effective_date), INTERVAL 2 DAY) FROM derived_aggregate_daily_senders_top10_rollup),
(SELECT MIN(effective_date) FROM aggregate_report)) 
AND
COALESCE((SELECT effective_date FROM aggregate_report  WHERE effective_date >= COALESCE((SELECT DATE_SUB(max(effective_date), INTERVAL 2 DAY) FROM derived_aggregate_daily_senders_top10_rollup),
(SELECT MIN(effective_date) FROM aggregate_report)) ORDER BY effective_date ASC LIMIT 50000,1), CURRENT_TIMESTAMP)
group by ddt.parent_id,effective_date,source_ip;


UPDATE `derived_job_semaphore` SET end_time=CURRENT_TIMESTAMP where `job_type` = 'derived_aggregate_daily_senders_top10_rollup' and `start_time` = @started_at;
