...
CREATE TABLE IF NOT EXISTS `derived_aggregate_daily_rollup_20180427` (
...
CREATE PROCEDURE update_derived_aggregate_daily_rollup_20180427()
BEGIN
...
INSERT INTO `derived_job_semaphore_20180427` (job_type, start_time) VALUES ('derived_aggregate_daily_rollup',@started_at);




REPLACE INTO `derived_aggregate_daily_rollup_20180427` 
...
FROM `derived_aggregate_daily_20180427` dad
JOIN derived_domain_tree_20180427 ddt ON ddt.child_id = dad.domain_id
where effective_date >= COALESCE((SELECT DATE_SUB(max(effective_date), INTERVAL 2 DAY) FROM derived_aggregate_daily_rollup_20180427),
(SELECT MIN(effective_date) FROM aggregate_report)) 
group by ddt.parent_id,effective_date;

UPDATE `derived_job_semaphore_20180427` SET end_time=CURRENT_TIMESTAMP where `job_type` = 'derived_aggregate_daily_rollup' and `start_time` = @started_at;
END IF;
