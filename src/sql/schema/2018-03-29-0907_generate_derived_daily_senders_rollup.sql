...
DROP TABLE IF EXISTS `derived_aggregate_daily_senders_top10_rollup`;
...
DROP PROCEDURE IF EXISTS generate_derived_aggregate_daily_senders_top10_rollup;
...
where effective_date >= COALESCE((SELECT DATE_SUB(max(effective_date), INTERVAL 2 DAY) FROM derived_aggregate_daily_senders_top10_rollup),
