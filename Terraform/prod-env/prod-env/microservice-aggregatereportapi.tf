module "aggregatereportapi" {
  ...
  registry-url              = "${var.ecr-aws-account-id}.dkr.ecr.${var.aws-region}.amazonaws.com/${var.env-name}/aggregatereportapi:${var.dotnet-container-githash}"
  command                   = "Dmarc.AggregateReport.Api.dll"
  service-name              = "aggregatereportapi"
  container-memory          = "${var.default-container-memory}"
  connection-string         = "Server = ${aws_rds_cluster.rds-cluster.endpoint}; Port = 3306; Database = ${var.db-name}; Uid = ${var.env-name}_${lookup(var.db-users,"statsapi")};Connection Timeout=5;"
  ...
  target-group-arn = "${element(module.loadbalancer-internal.target-group-arns, index(module.loadbalancer-internal.target-group-paths, "aggregatereport"))}"
}
